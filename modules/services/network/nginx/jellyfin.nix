...

{
  ...
  options = {
    modules.services.network.nginx.jellyfin.enable = 
      lib.mkEnableOption "Nginx proxy for jellyfin";
  };


  config = lib.mkIf (config.modules.services.network.nginx.jellyfin.enable) {

    services.nginx.virtualHosts."test.pakisrv.com" = {
